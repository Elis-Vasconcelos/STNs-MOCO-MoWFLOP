"""Roda ``scripts/metrics_stn_mowflop.R`` num dataset do MoWFLOP sem editar o arquivo R.

Mesmo padrão de ``run_scripts/run_metrics_r.py``: copia o script pra um
arquivo temporário, reescreve só as constantes ``iset``/``algo``
(``infolder`` é derivado dessas duas no próprio script), confere com um
diff que nada mais mudou, roda a cópia.
...
Uso::
    ...
    python -m mowflop.run_scripts.run_stn_metrics_r --tag g1.0 --algo MOEAD
    python -m mowflop.run_scripts.run_stn_metrics_r --tag x60 --algo NSGA2
"""
...
def rewrite(source: str, values: dict[str, str]) -> tuple[str, list[int]]:
    """Reescreve só ``iset``/``algo`` no texto-fonte de ``metrics_stn_mowflop.R``.

    Args:
        source: conteúdo original de ``metrics_stn_mowflop.R``.
        values: novos valores para ``iset`` e ``algo``.

    Returns:
        Tupla ``(fonte reescrita, números das linhas alteradas)``.

    Raises:
        ValueError: se alguma das duas constantes não for encontrada no fonte.
    """
    ...
    if missing:
        raise ValueError(f"could not find the constants in metrics_stn_mowflop.R: {sorted(missing)}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(diff) != 2 * len(CONST_LINES):
        print("refusing to run: more than the two constants differ", file=sys.stderr)
        return 1
